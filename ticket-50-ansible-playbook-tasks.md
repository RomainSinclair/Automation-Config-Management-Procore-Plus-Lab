...
Create and run an Ansible playbook targeting dev-app that creates a local user tfleming, expires that user's password immediately, and installs the tmux package.

## Requirements

- VM: dev-app (managed node)

- Ansible playbook must: create local user tfleming, expire tfleming's password, install tmux
...
- Expected: user tfleming exists, password expired (Last password change: Jan 01, 1970), tmux version printed
...
***Figure 1: Ansible control node (dev-ansible) — SSH connectivity established to dev-app-rs1***

***Figure 2: Ansible playbook execution — tasks running against dev-app***

***Figure 3: Ansible inventory showing dev_rs1 group with managed nodes***
